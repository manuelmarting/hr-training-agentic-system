...
def extract_personal_fact() -> str:
    ...
    channel preference). No-ops if nothing is found."""
    raise NotImplementedError("dispatched by orchestrator.py; never invoked directly")


@tool
def compose_delivery(closing: bool = False) -> str:
    """Compose and send this turn's actual reply to the employee, tailored to their
    detected sentiment/language and the channel. This is the only way anything
    reaches the employee. Set `closing=True` when this reply is today's wrap-up
